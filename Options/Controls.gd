...
const INPUT_ACTIONS = [ "move_up", "move_down"]
...
func wait_for_input(action_bind):
	...
	button = get_node("CenterContainer/VBoxContainer/bindings").get_node(action).get_node("Button")
...
func _input(event):
	...
	if event is InputEventKey:
		...
		get_node("CenterContainer/VBoxContainer/contextual_help").text = "Click a key binding to reassign it, or press the Cancel action."
...
func _ready():
	...
	for action in INPUT_ACTIONS:
		...
		# See note at the beginning of the script
		var button = get_node("CenterContainer/VBoxContainer/bindings").get_node(action).get_node("Button")
